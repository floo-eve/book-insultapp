...
//		String words[][] = {{"Artless", "Bawdy", "Beslubbering"}, {"Base-court", "Bat-fowling", "Beef-witted"}, {"Apple-john", "Baggage", "Barnacle"}};
		...
		try {
			...
			Connection connection = DriverManager.getConnection = DriverManager.getConnection(databaseURL,username,password);
			...
			if(connection!=null) {
				...
				while(rs.next()) {
					if(vowels.indexOf(rs.getString("first").chartAt(0))==-1) {
						article = "a";
					}
					theInsult = String.format("Thou art %s %s %s %s!", article, rs.getString("first"), rs.getString("second"),
					rs.getString("noun"));
